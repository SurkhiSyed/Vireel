...
def summarize_article(content):
    ...
    try:
        ...
        prompt = f"Provide a brief context in a few words, then summarize the following news article in about 150-200 words:\n\n{content}"
...
def summarize_articles(articles):
    ...
    for index, article in enumerate(articles):
        ...
        if not content or content == 'No content available':
            logger.warning(f"Article {index + 1} has no valid content, skipping summarization")
            summaries.append(article)  # Add the original article without summarization
            continue
        ...
        if summary:
            ...
            logger.warning(f"Failed to summarize article {index + 1}, using original content")
